...
// ============================================================================
// GC Frequency and Allocation Counter Tests
// ============================================================================

#[test]
fn test_gc_frequency_default() {
    let arena: Arena<isize, 10> = Arena::new(0);
    assert_eq!(arena.gc_frequency(), 0);
    assert_eq!(arena.alloc_count(), 0);
    assert!(!arena.should_collect());
}

#[test]
fn test_gc_frequency_set_and_get() {
    let arena: Arena<isize, 10> = Arena::new(0);
    arena.set_gc_frequency(500);
    assert_eq!(arena.gc_frequency(), 500);
    arena.set_gc_frequency(0);
    assert_eq!(arena.gc_frequency(), 0);
}

#[test]
fn test_alloc_counter_increments() {
    let arena: Arena<isize, 10> = Arena::new(0);
    assert_eq!(arena.alloc_count(), 0);
    arena.alloc(1).unwrap();
...
#[test]
fn test_alloc_contiguous_increments_counter() {
    let arena: Arena<isize, 20> = Arena::new(0);
    arena.alloc_contiguous(5, 0).unwrap();
    assert_eq!(arena.alloc_count(), 5);
    arena.alloc_contiguous(3, 0).unwrap();
    assert_eq!(arena.alloc_count(), 8);
}

#[test]
fn test_should_collect_threshold() {
    let arena: Arena<isize, 10> = Arena::new(0);
    arena.set_gc_frequency(3);
    assert!(!arena.should_collect());
    arena.alloc(1).unwrap();
    assert!(!arena.should_collect());
    arena.alloc(2).unwrap();
...
#[test]
fn test_should_collect_respects_gc_disabled() {
    let arena: Arena<isize, 10> = Arena::new(0);
    arena.set_gc_frequency(1);
    arena.alloc(1).unwrap();
    assert!(arena.should_collect());
    arena.set_gc_enabled(false);
    assert!(!arena.should_collect());
...
#[test]
fn test_should_collect_zero_frequency() {
    let arena: Arena<isize, 10> = Arena::new(0);
    arena.set_gc_frequency(0);
    arena.alloc(1).unwrap();
    arena.alloc(2).unwrap();
...
#[test]
fn test_reset_alloc_counter() {
    let arena: Arena<isize, 10> = Arena::new(0);
    arena.alloc(1).unwrap();
    arena.alloc(2).unwrap();
    assert_eq!(arena.alloc_count(), 2);
    arena.reset_alloc_counter();
    assert_eq!(arena.alloc_count(), 0);
}

#[test]
fn test_clear_resets_alloc_counter() {
    let arena: Arena<isize, 10> = Arena::new(0);
    arena.alloc(1).unwrap();
    arena.alloc(2).unwrap();
    assert_eq!(arena.alloc_count(), 2);
...
#[test]
fn test_gc_resets_alloc_counter() {
    let arena: Arena<Leaf, 20> = Arena::new(Leaf(0));
    let root = arena.alloc(Leaf(1)).unwrap();
    let _garbage1 = arena.alloc(Leaf(2)).unwrap();
    let _garbage2 = arena.alloc(Leaf(3)).unwrap();
    assert_eq!(arena.alloc_count(), 3);
...
#[test]
fn test_gc_frequency_with_collection() {
    let arena: Arena<Leaf, 20> = Arena::new(Leaf(0));
    arena.set_gc_frequency(3);
    let root = arena.alloc(Leaf(1)).unwrap();
    arena.alloc(Leaf(2)).unwrap(); // garbage
    arena.alloc(Leaf(3)).unwrap(); // garbage
...
#[test]
fn test_sweep_rebuilds_free_list_correctly() {
    let arena: Arena<Leaf, 10> = Arena::new(Leaf(0));
    let root = arena.alloc(Leaf(1)).unwrap();
    let _g1 = arena.alloc(Leaf(2)).unwrap();
    let _g2 = arena.alloc(Leaf(3)).unwrap();
    let _g3 = arena.alloc(Leaf(4)).unwrap();

    arena.collect_garbage(&[root]);
    assert_eq!(arena.len(), 1);

    // Free list should be valid after sweep
    assert!(arena.validate());

    // Should be able to allocate all freed slots
    for i in 0..9 {
        arena.alloc(Leaf(i)).unwrap();
    }
    assert_eq!(arena.len(), 10);
}
