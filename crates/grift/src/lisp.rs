...
impl<const N: usize> Lisp<N> {
    ...
    /// Set (mutate) an existing binding in the environment or its ancestors.
    ///
    /// Per Kernel §3.1, this walks the improper ancestor chain to find an
    /// existing binding of `name` and updates its referent to `val`.
    /// Signals an error if no binding is found.
    /// If `protected_env` is not nil, refuses to mutate that environment
    /// (ground environment protection per §3.2).
    pub(crate) fn env_set(
        &self,
        env: ArenaIndex,
        name: ArenaIndex,
        val: ArenaIndex,
        protected_env: ArenaIndex,
    ) -> ArenaResult<()> {
        self.env_set_dfs(env, name, val, ArenaIndex::NIL, protected_env)
    }

    /// Depth-first search to find and mutate an existing binding.
    fn env_set_dfs(
        &self,
        env: ArenaIndex,
        name: ArenaIndex,
        ...
        // Search local bindings for the name; if found, mutate the pair.
        let mut cur = bindings;
        while !cur.is_nil() {
            let binding = self.car(cur)?;
            if self.car(binding)? == name {
                // Protect the ground environment from mutation (§3.2).
                if !protected_env.is_nil() && env == protected_env {
                    return Err(ArenaError::ImmutableEnvironment);
                }
                // Mutate the binding pair's cdr to the new value.
                let binding_car = self.car(binding)?;
                return self.arena.set(binding, Value::Cons { car: binding_car, cdr: val });
            }
            cur = self.cdr(cur)?;
        }
        ...
        while !parent_cur.is_nil() {
            ...
            match self.env_set_dfs(parent_env, name, val, new_visited, protected_env) {
                Ok(()) => return Ok(()),
